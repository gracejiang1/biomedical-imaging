...
%%
% create function of parameter t, element wise multiplication for array
f = @(t) t.^2 ;
...
imagesc(mri_rat(:,:,5)) %look at 5th slice
...
imagesc([1:sy]*res_y,[1:sx]*res_x,mri_rat(:,:,10)) %create array from 1 to 256 and multiply by voxel so it ranges in mm from 0 to 44
...
montage(mri_rat)
%% (17) How can we place ROIs (region of interest) and then display the ROIs?
%%
close all
imagesc(mri_rat(:,:,10))
axis image;
...
[brain_mask, brain_x, brain_y] = roipoly(); %logic mask of 0 and 1
...
mean_brain = mean(slice_data(brain_mask(:)==1)) % take nonzero elements in brain_mask
